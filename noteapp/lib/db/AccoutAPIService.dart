...
class AccountAPIService {
  ...
  final String baseUrl = 'https://my-json-server.typicode.com/hoangtunuocmat/testflutter';
  ...
  Future<Account?> loginWithGoogle(String email, String displayName) async {
    final accounts = await getAllAccounts();
    try {
      // Check if the email already exists
      final existingAccount = accounts.firstWhere((account) => account.email == email);

      // Update the last login time
      existingAccount.lastLogin = DateTime.now().toIso8601String();
      await updateAccount(existingAccount);

      return existingAccount;
    } catch (e) {
      // If no account is found, create a new one with userId incremented by 1
      // Find the highest userId and increment by 1, or start with 1 if no accounts exist
      final newUserId = accounts.isNotEmpty
          ? accounts.map((account) => account.userId).reduce((a, b) => a > b ? a : b) + 1
          : 1;
